$signals = Get-Content "signals_final_working.json" -Raw | ConvertFrom-Json

Write-Host "Backtesting FINAL WORKING strategy..." -ForegroundColor Yellow
...
Write-Host "    FINAL WORKING STRATEGY RESULTS" -ForegroundColor Cyan
...
if ($trades.Count -gt 0) {
    ...
    for ($i = 0; $i -lt $trades.Count; $i++) {
        $runningBalance = $runningBalance * (1 + $trades[$i].Profit / 100)
        if ($runningBalance -gt $peak) { $peak = $runningBalance }
    ...
    $trades | Export-Csv -Path "trades_final_working.csv" -NoTypeInformation
    Write-Host "`nTrades saved to trades_final_working.csv" -ForegroundColor Gray
